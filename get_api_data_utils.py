...
def get_df_column_types(clientid, integrationid):
    try:
        df = pd.read_sql(f"select column_name, df_column_type from integration_columns where active and client_id = (select max(ifnull(client_id = {clientid},0))*{clientid} from integration_columns) and integration_id = {integrationid} and df_column_type is not null order by df_column_type ", con=engine)
    except Error as e:
...
def get_db_abc_plans(clubID):
    try:
        df = pd.read_sql(f"select planid,planname from abc_plans where club = {clubID}", con=engine)
    except Error as e:
        print("Error reading abc plan data:", e)
    return df
...
def summ_abc_pos_trans():
    ...
    where transactiontimestamp > (select coalesce(max(transactiontimestamp), '2020-01-01') from abc_pos_trans)
    group by employeeid	,
    etlrunid	,
    homeclub	,
    inventorytype	,
...
def upsert_items():
    ...
    upsert_sql = """
    INSERT INTO abc_items (etlrunid, club, itemid, itemname, itemtype, producttype, itemunitprice, itemupc, instock, minquantity, maxquantity, defaultquantity, itemcategoryname, itemdescription)
    SELECT etlrunid, club, itemid, itemname, itemtype, producttype, itemunitprice, itemupc, instock, minquantity, maxquantity, defaultquantity, itemcategoryname, itemdescription FROM staging.abc_items
    ON DUPLICATE KEY UPDATE
        etlrunid =  values(etlrunid),
        club =  values(club),
        itemname =  values(itemname),
        itemtype =  values(itemtype),
        producttype =  values(producttype),
        itemunitprice =  values(itemunitprice),
        itemupc =  values(itemupc),
    ...
    with connection.cursor() as cursor:
        create_table_query = """
            CREATE TABLE `abc_items` (
              ...
              `itemunitprice` decimal(10.2) DEFAULT NULL,
...
# rename columns
def rename_columns(df, clnid, intgid):

    rename_columns = get_db_integration_columns(clnid, intgid)
...
# rename columns
def convert_columns(df, clnid, intgid):

    convert_columns = get_df_column_types(clnid, intgid)

    filtered = convert_columns[convert_columns['df_column_type'] == 'numeric']
    num_cols = filtered['column_name'].tolist()

    filtered = convert_columns[convert_columns['df_column_type'] == 'date']
    date_cols = filtered['column_name'].tolist()

    filtered = convert_columns[convert_columns['df_column_type'] == 'datetime']
    datetime_cols = filtered['column_name'].tolist()

    filtered = convert_columns[convert_columns['df_column_type'] == 'bool']
    bool_cols = filtered['column_name'].tolist()
...
def upsert_plans():
    ...
    upsert_sql = """
    INSERT INTO abc_plans (etlrunid, club, planid, planname, promocode, promoname, agreementdescription, planstartdate, planenddate, onlineplandisplaylocation, additionalmembersallowed, mobilepaymentplan, limitedavailability, corporateplanonly)
    SELECT etlrunid, club, planid, planname, promocode, promoname, agreementdescription, planstartdate, planenddate, onlineplandisplaylocation, additionalmembersallowed, mobilepaymentplan, limitedavailability, corporateplanonly FROM staging.abc_plans
    ON DUPLICATE KEY UPDATE
        etlrunid = values(etlrunid),
        ...
        onlineplandisplaylocation = values(onlineplandisplaylocation),
        additionalmembersallowed = values(additionalmembersallowed),
        mobilepaymentplan = values(mobilepaymentplan),
        limitedavailability = values(limitedavailability),
        corporateplanonly = values(corporateplanonly);
    """
...
def upsert_plan():
    ...
    # update the rows and columns
    upsert_sql = """
    INSERT INTO abc_plan (planname, planid, activestatus, promotioncode, promotionname, membershiptype, agreementterm, schedulefrequency, terminmonths, dueday, firstduedate, activepresale, expirationdate, onlinesignupallowedpaymentmethods, preferredpaymentmethod, totalcontractvalue, downpaymentname, downpaymenttotalamount, scheduletotalamount, agreementterms, agreementdescription, agreementnote, clubfeetotalamount, planvalidation, onlineplandisplaylocation, corporateplanonly, additionalmembersallowed, renewalinformation_term, renewalinformation_paymentamount, renewalinformation_renewalfrequency, itemname, subtotal, tax, total, downpaymentid, recordtype, club, etlrunid)
    SELECT planname, planid, activestatus, promotioncode, promotionname, membershiptype, agreementterm, schedulefrequency, terminmonths, dueday, firstduedate, activepresale, expirationdate, onlinesignupallowedpaymentmethods, preferredpaymentmethod, totalcontractvalue, downpaymentname, downpaymenttotalamount, scheduletotalamount, agreementterms, agreementdescription, agreementnote, clubfeetotalamount, planvalidation, onlineplandisplaylocation, corporateplanonly, additionalmembersallowed, renewalinformation_term, renewalinformation_paymentamount, renewalinformation_renewalfrequency, itemname, subtotal, tax, total, downpaymentid, recordtype, club, etlrunid
    FROM staging.abc_plan
    ON DUPLICATE KEY UPDATE
        planname = values(planname),
        ...
        agreementterm = values(agreementterm),
        schedulefrequency = values(schedulefrequency),
        terminmonths = values(terminmonths),
        dueday = values(dueday),
        firstduedate = values(firstduedate),
        activepresale = values(activepresale),
        expirationdate = values(expirationdate),
        onlinesignupallowedpaymentmethods = values(onlinesignupallowedpaymentmethods),
        preferredpaymentmethod = values(preferredpaymentmethod),
        totalcontractvalue = values(totalcontractvalue),
        downpaymentname = values(downpaymentname),
        downpaymenttotalamount = values(downpaymenttotalamount),
        scheduletotalamount = values(scheduletotalamount),
        agreementterms = values(agreementterms),
        agreementdescription = values(agreementdescription),
        agreementnote = values(agreementnote),
        clubfeetotalamount = values(clubfeetotalamount),
        planvalidation = values(planvalidation),
        onlineplandisplaylocation = values(onlineplandisplaylocation),
        corporateplanonly = values(corporateplanonly),
        additionalmembersallowed = values(additionalmembersallowed),
        renewalinformation_term = values(renewalinformation_term),
        renewalinformation_paymentamount = values(renewalinformation_paymentamount),
        renewalinformation_renewalfrequency = values(renewalinformation_renewalfrequency),
        itemname = values(itemname),
...
def upsert_campaigns():
 ...
 INSERT INTO abc_campaigns (etlrunid, clubid,active,campaignid, campaignname,type)
    SELECT etlrunid, clubid, active,campaignid, campaignname,type FROM staging.abc_campaigns c 
    ON DUPLICATE KEY UPDATE
        etlrunid =  c.etlrunid,
        active = c.active,
